...
const ChatWindow = () => {
  ...
  useEffect(() => {
    ...
    const messagesChangeHandler = () => {
      ...
      if (
        ...
        newMessages[newMessages.length - 1].isWaiting
      ) {
        setLoading(true);
      } else {
        setLoading(false);
    ...
    ChatManager.instance.addEventListener(
      ChatManager.ACTIVE_SESSION_CHANGE_EVENT,
      activeSessionChangeHandler
    );
    ChatManager.instance.addEventListener(
      ChatManager.MESSAGES_CHANGE_EVENT,
      messagesChangeHandler
    );

    return () => {
      window.removeEventListener("orientationchange", handleOrientationChange);
      ChatManager.instance.removeEventListener(
        ChatManager.ACTIVE_SESSION_CHANGE_EVENT,
        activeSessionChangeHandler
      );
      ChatManager.instance.removeEventListener(
  ...
  const sendMessage = async () => {
    console.log(
      `Current session name ${ChatManager.instance.activeSession.name}, id: ${ChatManager.instance.activeSession.id}`
    );
